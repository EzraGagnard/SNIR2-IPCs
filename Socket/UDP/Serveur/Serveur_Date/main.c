...
int main(int argc, char** argv) {
    ...
    do {
        ...
        retourRecvfrom = recvfrom(serveurSocket, &date, sizeof (date), 0, (struct sockaddr *) &informationsClient, &tailleClient);
        if (retourRecvfrom == -1) {
        ...
        printf("Message de %s: %s %d %d %d\n", ip, date.jourDeLaSemaine,date.jour,date.mois,date.annee);
    } while (1);
    return (EXIT_SUCCESS);
}
