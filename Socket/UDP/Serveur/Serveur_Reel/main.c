...
int main(int argc, char** argv) {
   ...
   float monEntier;
    ...
    informationsServeur.sin_addr.s_addr = inet_addr(INADDR_ANY);
    ...
    retourRecvfrom = recvfrom(serveurSocket, &entierDuClient, sizeof (entierDuClient), 0, (struct sockaddr *) &informationsClient, &tailleClient);
    if (retourRecvfrom == -1) {
